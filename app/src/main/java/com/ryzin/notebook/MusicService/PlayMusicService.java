package com.ryzin.notebook.MusicService;
...
import com.ryzin.notebook.ScrollingActivity;
...
public class PlayMusicService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        switch (intent.getIntExtra("type", -1)) {
            case ScrollingActivity.PLAY_MUSIC:
                if (isStop) {
                    //重置mediaplayer
                    mediaPlayer.reset();
                    //将需要播放的资源与之绑定
                    mediaPlayer = MediaPlayer.create(this, R.raw.forgottentimes);
                    //开始播放
                    mediaPlayer.start();
                    //是否循环播放
                ...
                break;
            case ScrollingActivity.PAUSE_MUSIC:
                //播放器不为空，并且正在播放
                if (mediaPlayer != null && mediaPlayer.isPlaying()) {
                    mediaPlayer.pause();
                }
                break;
            case ScrollingActivity.STOP_MUSIC:
                if (mediaPlayer != null) {
                    //停止之后要开始播放音乐
                    mediaPlayer.stop();
                    isStop = true;
